...
# Lazy agent initialization to ensure env vars are fully loaded first
_agent = None
...
def chat():
    ...
    try:
        answer = get_agent().answer(question=question, mode=mode, sources=sources)
...
def summarize():
    ...
    try:
        ...
        summary = get_agent().summarize(sources=sources, filename=filename)
    except RuntimeError as e:
        return jsonify({"error": str(e)}), 500
